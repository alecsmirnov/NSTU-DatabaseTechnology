...
## QUERIES
Запросы к сделанным лабораторным работам. Теоретические лабораторные: 1, 2, 3; практические: 4, 5, 8, 9.   
&nbsp; 
...
1. Инициализация данных (ИД) – генерация и запись данных во все таблицы, участвующие в реализуемой схеме модели репликации;
2. Имитации работы системы (ИРС) – моделирование процесса работы информационной системы, выполняя, c определенной дискретностью (интервал в несколько секунд), операции: вставки, обновления, удаления;
3. Репликатор данных (РД) – перенос данных и обеспечение согласованного состояния таблиц. Фиксирование изменений в журнал содержимого таблиц.

##### Программа инициализации данных (ИД)
ИД записывает данные во все таблицы, участвующие в реализуемой схеме модели репликации:
...
ИРС c определенной дискретностью (интервал в несколько секунд) моделирует процесс работы информационной системы, выполняя следующие действия:
...
##### Программа репликатор данных (РД)
РД работает в соответствии с моделью репликации, определенной условиями (схема репликации, условие запуска репликатора, способ разрешения коллизий).  
