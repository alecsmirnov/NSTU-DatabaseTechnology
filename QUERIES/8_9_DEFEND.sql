...
SELECT info.*, info.order - info.total AS debt
FROM (SELECT out_r.*, out_r.cost * out_r.kol AS price, 
             COALESCE((SELECT SUM(r.cost * r.kol)
                       FROM r
                       WHERE r.n_cl = out_r.n_cl AND (r.date_order < out_r.date_order OR r.date_order = out_r.date_order AND CAST(SUBSTR(r.n_real, 2) AS INTEGER) <= CAST(SUBSTR(out_r.n_real, 2) AS INTEGER))), 0
                      ) AS order,
             COALESCE((SELECT SUM(r.cost * r.kol)
                       FROM r
                       WHERE r.n_cl = out_r.n_cl AND (r.date_pay < out_r.date_order OR r.date_pay = out_r.date_order AND CAST(SUBSTR(r.n_real, 2) AS INTEGER) <= CAST(SUBSTR(out_r.n_real, 2) AS INTEGER))), 0
                      ) AS total
      FROM r AS out_r
      ORDER BY out_r.n_cl, out_r.date_order, CAST(SUBSTR(out_r.n_real, 2) AS INTEGER)
     ) info;
